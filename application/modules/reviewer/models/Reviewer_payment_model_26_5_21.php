...
class Reviewer_payment_model extends CI_Model {
	...
	public function foreign_applcaition_details($id=null,$status=null)
	{
		$this->db->select('doc.*, doc.pd_id doc_id,doc.expiry_at license_validity_date, prof.user_ID, prof.role, prof.email, prof.name, prof.fname, prof.lname, prof.dob, prof.gender, prof.license_no, p.name profession_name, pt.payment_gross amount, pt.payment_status, pt.txn_id, rev.first_name rev_firsname,rev.last_name rev_lastname');
	...
	{
		$this->db->select("u.*,p.name profession_name,c.name college_of, uni.university_name,cou.countries_name, pt.payment_gross amount, pt.payment_status, pt.txn_id,rev.first_name rev_firsname,rev.last_name rev_lastname"); 
	...
	{
		$this->db->select("c.*, u.business_name,u.company_logo,u.email,u.status,cd.accreditation_no,pt.payment_gross amount, pt.payment_status, pt.txn_id, rev.first_name rev_firsname, rev.last_name rev_lastname, pro.name profession_name, cat.name category_name");
		$this->db->from($this->coursedocumentsTbl.' c');
		$this->db->join($this->cepTbl.' u','c.provider_id = u.provider_id');
		$this->db->join($this->tbl_cep_documents.' cd','c.provider_id = u.provider_id');
		$this->db->join($this->paymentTbl.' pt','u.reference_no = cd.reference_no','left');
...
//echo $this->db->last_query(); die;		
	...
	public function profcertificate($docid){
		$this->db->select("doc.*, prof.fname, prof.lname, prof.name, doc.pd_id doc_id, doc.reviewer_id rev_id, doc.review_accept_date,  doc.expiry_at, doc.reviewer_status rev_status, pt.payment_gross amount, pt.payment_status, pt.txn_id,rev.first_name rev_firsname,rev.last_name rev_lastname");
		$this->db->from($this->pDocTbl.' doc');
		$this->db->join($this->userTbl.' prof', 'doc.user_id = prof.user_ID','left');
		$this->db->join($this->professionTbl.' p', 'prof.profession = p.id','left');	
		$this->db->join($this->paymenttransactionTbl.' pt', 'doc.user_id = pt.user_id','inner');	
	...
	public function getprofessionaldetails($id){
		$this->db->select('u.*,p.name professionname');
		$this->db->from($this->userTbl.' u');
	...
	public function get_training_course_applcaition($where=null,$group_by=null)
	{
		...
		$this->db->join($this->paymentTbl.' pt','t.provider_id = pt.user_id');
		...
		$this->db->or_where('t.reviewer_status','2');
		if($where!=null){
			foreach ($where as $key => $value) {
				$this->db->where($key,$value);
			}
	...
	function get_cep_acc_application($count = false, $status=null, $reviewer_id=null,$document_for=null){
		$this->db->select("doc.*, doc.id doc_id, doc.reviewer_id rev_id, doc.reviewer_status rev_status, doc.accreditation_no,cep.business_name, cep.business_no, cep.email, cep.provider_id pid, pt.payment_gross amount, pt.payment_status, pt.txn_id,t4.first_name rev_firsname,t4.last_name rev_lastname"); 
		$this->db->from($this->tbl_cep_documents.' as doc');
		$this->db->join($this->cepTbl.' as cep','doc.provider_id = cep.provider_id','inner');
		$this->db->join($this->paymenttransactionTbl .' as pt','pt.user_id = doc.provider_id','inner');
		$this->db->join($this->adminTbl .' as t4','doc.reviewer_id = t4.user_ID','left');
	...
	function get_fp_application($count=null, $status=null, $reviewer_id=null, $document_for = null){ 
		...
		if($reviewer_id){
			...
			$this->db->where('doc.reviewer_id <','1');
		}
		$this->db->group_end();
	...
	function get_fpexam_application($count=null, $status=null, $reviewer_id=null, $type = null){ 
		$this->db->select('doc.*, doc.pd_id doc_id, 
			prof.user_ID, prof.role, prof.email, prof.name, prof.fname, prof.lname,
	...
	function get_fpexam_reg_application($count=null, $status=null, $reviewer_id=null, $type = null){ 
		$this->db->select('doc.*, doc.pd_id doc_id, 
			prof.user_ID, prof.role, prof.email, prof.name, prof.fname, prof.lname,
			p.name collegeofname, 
			b.examination_id,
	...
	function get_school_application($count=null, $status=null, $reviewer_id=null, $document_for=null){  
		$this->db->select('ud.unidoc_id,ud.business_license,ud.accreditation,ud.reviewer_id,ud.reviewer_status,ud.review_date,ud.document_for,ud.renew_for,ud.accreditation_number,ud.expiry_at,ud.review_accept_date,
